...
- Made the public-namespace test work in an uninstalled source checkout.

## [1.0.7] — 2026-09-14

### Fixed

- Corrected public-namespace test imports for the repository's isort check.

## [1.0.6] — 2026-09-14

### Fixed

- Corrected public-namespace test imports so the full CI lint matrix passes.

## [1.0.5] — 2026-09-14

### Added

- Public `probviz` import namespace with aliases for distributions, fitting,
  Monte Carlo, statistical tests, utilities, and visualizers.

### Fixed

- Editable-install test instructions now install the `test` extra.
- SciPy numerical warnings from unsuccessful distribution-fit candidates no
  longer leak through `DistributionFitter.fit_all()`.

## [1.0.4] — 2026-09-14

### Fixed

- README gallery images now use release-pinned URLs that render on PyPI.
- Gallery GIFs are included in source distributions for complete source checkouts.

## [1.0.3] — 2026-09-13

### Fixed

- `src/cli.py`: preserve original order when forwarding Streamlit flags
  (`--server.headless`/`--server.port`) — previously `unknown + REMAINDER`
  were concatenated in the wrong order (`unknown` appended instead of
  prepended).
